...
// PatternRegion is now used from regions.rs
...
fn extract_outline_regions(payload: &PdfExportPayload) -> Result<Vec<PatternRegion>, String> {
    let regions_payload = regions::RegionExtractionPayload {
        width: payload.width,
        height: payload.height,
        stitches: payload
            .stitches
            .iter()
            .map(|s| regions::RegionStitch {
                x: s.x,
                y: s.y,
        ...
        legend: payload
            .legend
            .iter()
            .map(|l| regions::RegionLegendEntry {
...
// Helper functions moved to regions.rs or kept if private/specific
